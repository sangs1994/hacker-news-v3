...
export default function JobsPage() {
  ...
  const openComments = React.useCallback(
    (storyId: number) => {
      // Use full loaded list so dialog works even with filtered/sorted view
      const story = stories.find((s) => s.id === storyId) ?? null;

      // Jobs often have no comments — guard to avoid opening empty dialog
  ...
  return (
    <Box sx={{ px: { xs: 2, md: 3 }, py: 3 }}>
      ...
      {isError && (
        ...
        >
          <Typography sx={{ fontWeight: 700 }}>Failed to load Jobs</Typography>
